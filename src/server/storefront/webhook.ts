...
import { jsonObject } from './checkout';
import {
  classifyWaylStatus,
  checkoutEventKey,
...
export async function processWaylWebhook(input: {
  ...
  try {
    const authoritative = await new WaylClient(input.config.wayl).getPaymentLink(referenceId);
    ...
    if (
      ...
      await prisma.$transaction([
        prisma.storefrontCheckout.update({
          where: { id: checkout.id },
          data: { status: 'REVIEW', reviewReason: 'wayl_reconciliation_mismatch', waylStatus: authoritative.status },
        }),
        prisma.waylWebhookEvent.update({
          where: { id: event.id },
          data: { checkoutId: checkout.id, status: 'IGNORED', errorCode: 'reconciliation_mismatch', processedAt: new Date() },
        }),
      ]);
      return { accepted: true, review: true };
    }
      ...
      if (statusClass === 'PAID' && current.status !== 'PAID') {
        ...
        await tx.auditLog.create({
          data: {
            ...
            metadata: { referenceId, amount: checkout.total, source: 'wayl-webhook' },
          },
        });
        ...
        await tx.storefrontCheckout.update({
          where: { id: checkout.id },
          data: { status: 'REVIEW', waylStatus: authoritative.status, reviewReason: 'paid_payment_changed' },
        });
        ...
        await tx.storefrontCheckout.update({
          where: { id: checkout.id },
          data: { status: 'REVIEW', waylStatus: authoritative.status, reviewReason: 'unknown_wayl_status' },
        });
      }
      await tx.waylWebhookEvent.update({
        where: { id: event.id },
        data: { checkoutId: checkout.id, status: 'SUCCEEDED', processedAt: new Date() },
      });
    });
    return { accepted: true, duplicate: false, status: statusClass };
  } catch (error) {
