...
export default async function ShareholdersPage({
  ...
  const [shareholders, capEntries, expenseEntries] = await Promise.all([
    ...
    prisma.financeEntry.findMany({ where: { type: 'CAPITAL_IN', currency: 'IQD' }, select: { partyId: true, amount: true } }),
    prisma.financeEntry.findMany({ where: { type: { in: ['EXPENSE', 'PURCHASE'] }, currency: 'IQD' }, select: { amount: true } }),
  ]);
