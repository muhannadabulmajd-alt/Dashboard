...
describe('storefront configuration', () => {
  it('accepts an isolated Preview configuration', () => {
    ...
    expect(config.wayl?.baseUrl).toBe('https://api.thewayl-staging.com');
  });

  it('accepts production only with the live Wayl host', () => {
  ...
  it('rejects Preview credentials pointed at production Wayl', () => {
    expect(() => readStorefrontConfig({ ...base, WAYL_API_BASE_URL: 'https://api.thewayl.com' }))
